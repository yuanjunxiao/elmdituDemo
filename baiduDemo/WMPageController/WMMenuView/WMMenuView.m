...
- (UIColor *)lineColor {
    if (!_lineColor) {
        _lineColor = XK_COL_RGB(0x2edead);
    }
    return _lineColor;
}
...
- (void)addProgressView {
    ...
    pView.color = self.lineColor.CGColor;//下滑条的颜色
    pView.backgroundColor = [UIColor whiteColor];//下滑条滑动范围的颜色
    self.progressView = pView;
    [self.scrollView addSubview:pView];
    
//    UIImageView *linwImage = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenSize.width/2-0.5, 10, 1, 20)];//XK_COL_RGB(0xa3a3a3)
//    linwImage.backgroundColor = XK_COL_RGB(0xa3a3a3);
//    [self.scrollView addSubview:linwImage];
